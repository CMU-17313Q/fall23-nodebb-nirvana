...
module.exports = function (User) {
    ...
    User.blocks.list = async function (uids) {
        ...
        if (unCachedUids.length) {
            const unCachedData = await db.getSortedSetsMembers(unCachedUids.map(uid => `uid:${uid}:blockedUids`));
    ...
    User.blocks.add = async function (targetUid, uid) {
        ...
        await db.sortedSetAdd(`uid:${uid}:blockedUids`, Date.now(), targetUid);
    ...
    User.blocks.remove = async function (targetUid, uid) {
        ...
        await db.sortedSetRemove(`uid:${uid}:blockedUids`, targetUid);
